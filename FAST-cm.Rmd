...
test.league.metrics <- merge(test.scores, brier[,c(1:4,7)],
                             by=c("model","infoset", 
                                  "covariates", "in_africa_me"))
  
tmp <- test.league.metrics %>% filter(in_africa_me=="Yes") %>%
  arrange(se_mean)
...
```{r finalmodel-cm, echo=TRUE, cache=FALSE, warning=FALSE}
rm(list=ls())
...
```{r regtable, echo=FALSE, warning=FALSE, cache=TRUE}
...
```{r covariate_explainer, echo=FALSE, fig.align='center', message=FALSE, fig.keep='last', fig.cap="Odds ratios for selected covariates in the NBGLMM."}
...
```{r effects, echo=TRUE, cache=FALSE, fig.keep='last', fig.cap="Average marginal effects for NBGLMM"}
...
Same as above, but with accounting for the random effects -- smaller effects and possibly messier to interpret,  but more realistic of the decision-maker's problem.
...
```{r forecastsummaries, echo=TRUE, cache=FALSE, warning=FALSE, message=FALSE}
...
This is a bit trickier than the totaling of the cumulative means.  The target prediction here is whether of not there are more than 25 deaths (so predicting when a cumulant of `ged_sb` > 25) in a target period.  

There is a "right" and a "wrong" way to do this computation and it matters since the probability of the cumulative event is not the cumulative of the probability of the event.  So one needs to take some care in this computation when considering it in a model and for a sample of forecasts.  (It is a classic case of wanting an expectation of a relevant function not a function of the expectation.)

```{r cumul_prob, echo=TRUE, message=FALSE}
...
```{r writeforcs2xlsx, echo=TRUE, cache=FALSE, message=FALSE}
