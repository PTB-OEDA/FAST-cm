...
p.glmm.off <- glmmTMB(frm.glmm,
                      offset = log(wdi_sp_pop_totl),
                      family = poisson,
                      data=train)
...
AIC(p.glmm); AIC(p.glmm.off)
AIC(nb.glmm); AIC(nb.glmm.off)
AIC(tw.glmm); AIC(tw.glmm.off)
...
p.off <- broom.mixed::tidy(p.glmm.off, effects = "fixed", exponentiate=TRUE) |> mutate(model = "PGLMM + COVAR + OFF")
nb.off <- broom.mixed::tidy(nb.glmm.off, effects = "fixed", exponentiate=TRUE) |> mutate(model = "NBGLMM + COVAR + OFF")
tw.off <- broom.mixed::tidy(tw.glmm.off, effects = "fixed", exponentiate=TRUE) |> mutate(model = "TWGLMM + COVAR + OFF")

pdf("modelselect-glmm-covar-figs.pdf", width=6, height=6)
dw <- dwplot(rbind(p.tmp[2:4,], nb.tmp[2:4,], tw.tmp[2:4,],
                   p.off[2:4,], nb.off[2:4,], tw.off[2:4,]))
print(dw+geom_vline(xintercept=1,lty=2))

dw2 <- dwplot(rbind(p.tmp[5,], nb.tmp[5,], tw.tmp[5,],
                    p.off[5,], nb.off[5,], tw.off[5,]))
...
eval.p.glmm.off <- predict(p.glmm.off, newdata=test, 
                       type = "response",
                       allow.new.levels = TRUE)

eval.nb.glmm.off <- predict(nb.glmm.off, newdata=test, 
                        type = "response",
                        allow.new.levels = TRUE)

eval.tw.glmm.off <- predict(tw.glmm.off, newdata=test, 
                        type = "response",
                        allow.new.levels = TRUE)
...
eval.p.glmm.off.mcmc <- sapply(1:n, function(i) {rpois(N, eval.p.glmm.off[i])})          
eval.nb.glmm.off.mcmc <- sapply(1:n, function(i) {rnbinom(N, size=sigma(nb.glmm.off), 
                                                      mu=eval.nb.glmm.off[i])})
eval.tw.glmm.off.mcmc <- t(replicate(N, mgcv::rTweedie(eval.tw.glmm.off, 
                                                   p=family_params(tw.glmm.off))))
...
f.p <- cbind(test, t(eval.p.glmm.off.mcmc))
P.off.stacked <- reshape(f.p, 
                     direction = "long",
                     varying = list(names(f.p)[4:(N+3)]),
                     v.names = "predicted",
                     idvar = c("month_id", "country_id"),
                     timevar = "sample_id",
                     times = 1:N)
P.off.stacked$model <- "Poisson GLMM + COVAR + OFF"
...
f.nb <- cbind(test, t(eval.nb.glmm.off.mcmc))
NB.off.stacked <- reshape(f.nb, 
                      direction = "long",
                      varying = list(names(f.nb)[4:(N+3)]),
                      v.names = "predicted",
                      idvar = c("month_id", "country_id"),
                      timevar = "sample_id",
                      times = 1:N)
NB.off.stacked$model <- "Neg Binom GLMM + COVAR + OFF"
...
f.tw <- cbind(test, t(eval.tw.glmm.off.mcmc))
TW.off.stacked <- reshape(f.tw, 
                      direction = "long",
                      varying = list(names(f.tw)[4:(N+3)]),
                      v.names = "predicted",
                      idvar = c("month_id", "country_id"),
                      timevar = "sample_id",
                      times = 1:N)
TW.off.stacked$model <- "Tweedie GLMM + COVAR + OFF"
