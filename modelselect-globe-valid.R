...
#### Define the dataset ####
train <- globe.valid
test <- globe.valid.eval

#### GLMs Estimations and Prediction ####

N <- 1000
xi <- seq(1.3,1.75,by=0.05)
globe.valid.frm.preds <- count.cf(frm=frm,
                                   train,
                                   test,
                                   xi.vec = xi,
                                   N=N)

globe.valid.frm1.preds <- count.cf(frm=frm1, 
                                   train,
                                   test,
                                   xi.vec = xi,
                                   N=N)

globe.valid.frm2.preds <- count.cf(frm=frm.covar,
                                   train,
                                   test,
                                   xi.vec = xi,
                                   N=N)
...
save(list=ls(pattern=".glmm"), file = "globe.valid.interim-glmm.RData")
rm(list=ls(pattern=".glmm"))
gc()

#### Check all fitted for scoring ####
colnames(globe.valid.frm.preds$forecasts)
colnames(globe.valid.frm1.preds$forecasts)
colnames(P.stacked)
colnames(NB.stacked)
colnames(TW.stacked)

#### Stacked and saved ####
globe.valid.frm.preds$forecasts$model <- paste(globe.valid.frm.preds$forecasts$model,
                                                   "+FE", sep="")
globe.valid.frm1.preds$forecasts$model <- paste(globe.valid.frm1.preds$forecasts$model,
                                                    "+LAGS", sep="")
globe.valid.frm2.preds$forecasts$model <- paste(globe.valid.frm2.preds$forecasts$model,
                                                     "+LAGS+COVAR", sep="")

forcs <- rbind(globe.valid.frm.preds$forecasts,
               globe.valid.frm1.preds$forecasts,
               globe.valid.frm2.preds$forecasts,
               P.stacked,
               NB.stacked,
               TW.stacked)

save.image("globe.valid.forcs.RData")
